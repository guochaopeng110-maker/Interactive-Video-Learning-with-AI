  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC]">
      <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
        <header className="rounded-[32px] bg-white px-8 py-10 shadow-sm">
          ...
          <p className="mt-4 max-w-3xl text-sm leading-7 text-slate-600">
            同一个账号可能拥有多个角色。这个页面把 role-based prototype 的信息架构显式化，让共享登录入口和后续工作台之间有一个清晰的分流节点。
          </p>
        </header>
          ...
          {roleMetaList.map((role) => (
            <article key={role.key} className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="mt-6 rounded-2xl bg-slate-50 p-4">
                <p className="text-xs uppercase tracking-[0.18em] text-slate-400">进入后将看到</p>
                <p className="mt-2 text-sm leading-6 text-slate-600">
                  {role.key === 'teacher' && 'Course、Material、KnowledgeGraph 和 Lesson 的角色化原型骨架。'}
                  {role.key === 'student' && '课程入口、学习主页、只读学习路径与后续 student flow 的共享壳层。'}
                  {role.key === 'admin' && '目录治理、用户权限与组织分析的独立管理壳层。'}
                </p>
              </div>
